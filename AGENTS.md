...
- **NS 学习路线**：进度统一记录在 `learning-todons-progress.md`，每完成一个话题追加会话日志；每天结束说"收尾"时汇总更新路线图状态；用户跳转时先检查停车场和路线偏差
...
- 所有生成产物保存到 `learning-todo` 及其子目录（图片默认 `learning-todoimgs/`），`--watermark false`
...
- 将正式输出指向到learning-todochats目录下的ipynb文件中, 可以按session id确定不同文件的归属session, 每次回复都追加新的markdown格式框, 不要大规模修改已经写好的文本框
