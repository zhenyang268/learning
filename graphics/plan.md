...
- **V1.2** 随机矩阵求逆残差 ‖A·A⁻¹ − I‖∞ < 1e-8（double）；复矩阵同样通过。
...
- **V3.2** `x' = Ax` 数值解与 `expm` 解析解差 < 局部截断误差理论阶（RK4 为 O(h⁴)：
  h 减半误差 ÷16 近似成立）。
...
2. 可视化：热方程 = 图像模糊（`e^{tΔ}` 半群视角，帧缓冲直接显示）；
   二维波动方程 = 鼓膜振动。
3. CG 求解过程可视化：每次迭代画能量泛函 Φ(x) = ½·x^T·A·x − b^T·x 下降 ——
   把「变分法求弱解」从公式变成动画。
